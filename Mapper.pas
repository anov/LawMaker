...
uses
  ...
  Q_Sectors,Q_Walls, Q_Objects, M_Tools, Stitch, Containers, CopyPaste, LECScripts;
...
Find_type=(f_first,f_next);
  TMapWindow = class(TForm)
    ...
    SpeedButtonMission: TSpeedButton;
    ...
    procedure SpeedButtonMissionClick(Sender: TObject);
...
{Display parameters}
  ...
  public
    ...
    Procedure PutObjectsOnFloor;
    Procedure StickObjectsToCeiling;
  end;
...
Try
  if LEVELloaded then
    with Map.Canvas do
      begin
         sel_color:= col_select;
        case MAP_MODE of
         MM_SC: if SC_MULTIS.FindSector(SC_HILITE)<>-1 then sel_color:=col_Select_multi;
         MM_WL: if WL_MULTIS.FindWall(SC_HILITE,WL_HILITE)<>-1 then sel_color:=col_Select_multi;
         MM_VX: if VX_MULTIS.FindVertex(SC_HILITE,VX_HILITE)<>-1 then sel_color:=col_Select_multi;
         MM_OB: if OB_MULTIS.FindObject(OB_HILITE)<>-1 then sel_color:=col_Select_multi;
        end;
        Pen.Color:=sel_color;
        if isxor then
         Pen.Mode := pmXor
        ...
        MM_SC: begin
                 ...
                 for j := 0 to TheSector.Walls.Count - 1 do
                   begin
                     TheWall := TheSector.Walls[j];
                     LVertex := TheSector.Vertices[TheWall.V1];
                     if j = 0 then DO_Draw_BigVX(LVertex.X, LVertex.Z, sel_color);
                     RVertex := TheSector.Vertices[TheWall.V2];
                     MoveTo(M2SX(LVertex.X), M2SZ(LVertex.Z));
                     LineTo(M2SX(RVertex.X), M2SZ(RVertex.Z));
                   end;
        ...
        MM_WL: begin
                 ...
                 LVertex := TheSector.Vertices[TheWall.V1];
                 DO_Draw_BigVX(LVertex.X, LVertex.Z, sel_color);
                 RVertex := TheSector.Vertices[TheWall.V2];
                 MoveTo(M2SX(LVertex.X), M2SZ(LVertex.Z));
                 LineTo(M2SX(RVertex.X), M2SZ(RVertex.Z));
                 DO_Draw_WLperp(SC_HILITE, WL_HILITE, sel_color);
               end;
        MM_VX: begin
                 Dimension := Round(vx_scale*SCALE/2);
                 if Dimension > vx_dim_max then Dimension := vx_dim_max;
                 ...
                 DO_Draw_BigVX(TheVertex.X, TheVertex.Z, sel_color);
               end;
        MM_OB: begin
...
begin
 {Create Sector/Wall/Vertex/Object editors}
  ...
  ScriptEditor1.ShortCut:=ShortCut(Ord('S'),[ssShift,ssCtrl]);
...
With SpeedButtonMission do
if newlevel then
begin
 Enabled:=false;
 Hint:='Diabled - save level first';
 MissionMenu.Enabled:=false;
end
else
begin
 Enabled:=true;
 Hint:='Edit mission attributes';
 MissionMenu.Enabled:=true;
end;
 ...
 if LEVELLoaded then
  begin
    ...
    Case Key of
      ...
      $4D {VK_M} : ;
      $4E {VK_N} : ;
      $50 {VK_P} : if Map_mode=MM_OB then PutObjectsOnFloor;
...
begin
 ...
 if LogTestSession then Cmd:=' /df testlog.txt '+ExtractName(LevelFile)
  else Cmd:=' '+ExtractName(LevelFile);
  CreateProcess(
...
begin
 if NewLevel then
 begin
  if IsInContainer(LevelSrcFile) then inf:=ChangeExt(LevelSrcFile,'.inf') else exit;
 end
 else Inf:=ChangeFileExt(LevelFile,'.inf');
 ScriptEdit.OpenScript(Inf);
end;
...
begin
 if GridON then
  begin
   ...
   if X < -Grid / 2 then GX := GX - Grid;
   if Z < -Grid / 2 then GZ := GZ - Grid;
...
begin
 ...
 begin
  ...
  if IsInContainer(LevelSRCFile) then
  begin
   Try
   CopyAllFile(ChangeExt(LevelSRCFile,'.inf'),ChangeExt(LevelFile,'.inf'));
   except
    On e:Exception do PanMessage(mt_warning,e.Message);
   end;
   Try
   CopyAllFile(ChangeExt(LevelSRCFile,'.itm'),ChangeExt(LevelFile,'.itm'));
   except
    On e:Exception do PanMessage(mt_warning,e.Message);
   end;
  end;
...
Procedure TMapWindow.SetGrid(size:double);
begin
 if size>256 then size:=256
 else if size<0.25 then size:=0.25;
 if grid<>size then
 begin
  Grid:=size;
  PanelGrid.Caption := FloatToStr(Grid);
...
begin
 CASE MAP_MODE of
  MM_SC : if SC_Multis.Count=0 then TranslateSector(TheLevel,SC_HILITE,x,z)
           ...
           begin
            i:=SC_MULTIS.AddSector(SC_HILITE);
            TranslateSectorSelection(TheLevel,SC_Multis,x,z);
            SC_MULTIS.Delete(i);
           end;
  MM_WL : if WL_Multis.Count=0 then TranslateWall(TheLevel,SC_HILITE,WL_HILITE,x,z)
          else
          begin
           i:=WL_MULTIS.AddWall(SC_HILITE,WL_HILITE);
           TranslateWallSelection(TheLevel,WL_Multis,x,z);
           WL_MULTIS.Delete(i);
          end;
  MM_VX : if VX_Multis.Count=0 then TranslateVertex(TheLevel,SC_HILITE,VX_HILITE,x,z)
          else
          begin
           if VX_MULTIS.FindVertex(SC_HILITE,VX_HILITE)=-1 then
            i:=VX_MULTIS.AddVertex(SC_HILITE,VX_HILITE) else i:=-1;

           TranslateVertexSelection(TheLevel,VX_Multis,x,z);
           if i<>-1 then VX_MULTIS.Delete(i);
          end;
  MM_OB : if OB_Multis.Count=0 then TranslateObject(TheLevel,OB_HILITE,x,z)
          else
          begin
           TranslateObjectSelection(TheLevel,OB_Multis,x,z);
           if OB_MULTIS.FindObject(OB_HILITE)=-1 then TranslateObject(TheLevel,OB_HILITE,x,z);
          end;
 END;
MODIFIED := TRUE;
Map.Invalidate;
...
begin
 Result:=true;
  ...
  begin
   ...
   if (ext<>'.LVT') and (ext<>'.OBT') and
      ...
      (ext<>'.PHY') and (ext<>'.LAF') and
      (ext<>'.TXT') Then Files.Delete(i);
  end;
  {Create LAB}
 ...
 begin
  ...
  Except
   On e:EInOutError do PanMessage(mt_Error,SysErrorMessage(e.ErrorCode));
  end;
 end;
end;
...
var sr:TSearchRec;
    RCSName,RCAName,RCMName,ITMName,CHKName:String;
begin
 if not levelLoaded then begin ShowMessage('Load or create a level first'); exit; end;
 if Modified or NewLevel then begin ShowMessage('Save the level first'); exit; end;
 if FindFirst(ProjectDirectory+'*.rcs',faAnyFile,sr)=0 then RCSName:=sr.Name else RCSName:='';
 SysUtils.FindClose(sr);
 if FindFirst(ProjectDirectory+'*.rca',faAnyFile,sr)=0 then RCAName:=sr.Name else RCAName:='';
 SysUtils.FindClose(sr);
 if FindFirst(ProjectDirectory+'*.rcm',faAnyFile,sr)=0 then RCMName:=sr.Name else RCMName:='';
 SysUtils.FindClose(sr);
 ITMName:=LevelName+'.itm';
 if not FileExists(ProjectDirectory+ITMName) then ITMName:='';
 if ITMName='' then CHKName:='' else
  CHKName:=GetCHKFromITM(ProjectDirectory+ITMName);
 if CHKName<>'' then
  if not FileExists(ProjectDirectory+CHKName) then CHKName:='';
 if (RCSName='') and (RCAName='') and (RCMName='') then
 begin
  If MsgBox('Your level does not have RCS, RCA or RCM file! Use Mission editor to create one ("Mission" button in "Level" tab). Publish anyway?','Confirm',MB_YESNO)=idNo
  then exit;
 end;
 if RCMName<>'' then
  if (ITMName='') or (CHKName='') then
  begin
  if MsgBox('Your multi player level does not have security files: ITM and CHK! Use Mission editor to create them ("Mission" button in "Level" tab). Publish anyway?','Confirm',MB_YESNO)=idNo
  then exit;
  end;
 PublishDialog.FileName:=GameDir+ChangeFileExt(ExtractFileName(LevelFile),'.lab');
 if PublishDialog.Execute then
 begin
  if CompareText(ExtractFilePath(PublishDialog.FileName),ProjectDirectory)=0 then
    if MsgBox('You''re trying to publish to your project directory. This isn''t a good idea. Want to continue?','Warning!',mb_YesNo)=idNo then exit;
  DO_Publish(PublishDialog.FileName);
 end;
end;
...
Procedure TMapWindow.PutObjectsOnFloor;
Var TheObject:TOB;
    Layered:Integer;
    i:Integer;
begin
 ...
 Layered:=0;
 TheObject:=TheLevel.Objects[OB_HILITE];
 if PutOnFloor(TheLevel,Layer,OB_HILITE) then Inc(Layered);

 for i:=0 to OB_MULTIS.count-1 do
  PutOnFloor(TheLevel,Layer,OB_MULTIS.GetObject(i));

 SetCurOB(OB_HILITE);
 if layered<>0 then MODIFIED:=true;
 PanelText.Caption := IntToStr(Layered) + ' OBJECT(S) PUT ON FLOOR';
end;

Procedure TMapWindow.StickObjectsToCeiling;
Var TheObject:TOB;
    Layered:Integer;
    i:Integer;
begin
 ...
 Layered:=0;
 TheObject:=TheLevel.Objects[OB_HILITE];
 if PutOnCeiling(TheLevel,Layer,OB_HILITE) then Inc(Layered);

 for i:=0 to OB_MULTIS.count-1 do
  PutOnCeiling(TheLevel,Layer,OB_MULTIS.GetObject(i));

 SetCurOB(OB_HILITE);
 if layered<>0 then MODIFIED:=true;
 PanelText.Caption := IntToStr(Layered) + ' OBJECT(S) PUT ON CEILING';
end;
...
begin
 case Map_Mode of
  ...
  MM_WL: begin
          ...
          if n=-1 then n:=WL_MULTIS.AddWall(SC_HILITE,WL_HILITE) else N:=-1;
  ...
  MM_VX: begin
          ...
          if n=-1 then n:=VX_MULTIS.AddVertex(SC_HILITE,VX_HILITE) else N:=-1;
...
begin
 ...
 begin
  if what and d_floors<>0 then
   Floor_Y:=RoundTo2Dec(First_FY+FY_Dif*m/(ns-1));
  if what and d_ceilings<>0 then
   Ceiling_Y:=RoundTo2Dec(First_CY+CY_Dif*m/(ns-1));
  if what and d_Ambients<>0 then
   Ambient:=Round(FirstAmb+AmbDif*m/(ns-1));
 end;
Modified:=true;
Do_Fill_SectorEditor;
end;
...
begin
 Case map_mode of
  ...
  MM_WL: begin
          ...
          p:=pos(',',s); if p=0 then begin ShowMessage('Invalid number');exit; end;
          sc:=StrToInt(Copy(s,1,p-1));
          wl:=StrToInt(Copy(s,p+1,length(s)-p));
...
procedure TMapWindow.SpeedButtonMissionClick(Sender: TObject);
begin
 MissionEdit.EditMission;
end;

Function TMapWindow.GetLevelName:String;
begin
 Result:=ChangeFileExt(ExtractName(LevelFile),'');
end;

Procedure TMapWindow.ExceptHandler(Sender: TObject; E: Exception);
begin
 if E is EInOutError then ShowMessage(SysErrorMessage(EInOutError(E).ErrorCode))
  else Application.ShowException(e);
end;

Procedure TMapWindow.DrawLine(x1,y1,x2,y2:integer);
var dx,dy,a:integer;
begin
 dx:=x2-x1;
 dy:=y2-y1;
 if (x1<0) or (x1>ScreenX) then
 begin
 end;
end;
